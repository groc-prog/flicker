...
CREATE TYPE "translation_locale" AS ENUM('de', 'en');--> statement-breakpoint
...
CREATE TABLE "movie_translations" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
	"language" "translation_locale" NOT NULL,
	"title" text NOT NULL,
	"description" text,
	"videos" jsonb,
	"homepage" text,
	"movie_id" uuid,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "movie_translations_language_movie_id_unique" UNIQUE("language","movie_id")
);
--> statement-breakpoint
CREATE TABLE "movies" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
	...
	"scraped_movie_id" uuid NOT NULL UNIQUE,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
CREATE INDEX "idx_movie_translations_language_title" ON "movie_translations" USING gin ("language", "title" gin_trgm_ops);--> statement-breakpoint
...
ALTER TABLE "movie_translations" ADD CONSTRAINT "movie_translations_movie_id_movies_id_fkey" FOREIGN KEY ("movie_id") REFERENCES "movies"("id") ON DELETE SET NULL;--> statement-breakpoint
